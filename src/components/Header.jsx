        ...
        <div className='w-[100vw] z-10 h-[10vh] max-sm:h-[8vh] dark:bg-black dark:text-white border-b-[.1vw] dark:border-b-ligthBlack dark:border-lightBlack flex justify-between items-center px-[5vw]'>
            ...
            <div className='flex justify-center items-center gap-[1vw] max-sm:gap-[1.5vw]'>
                <button onClick={() => dispatch(darkMode())} className="active:scale-95">
                    {darkModeData ? (
                        <BiSun className="font-semibold  text-[2.2vw] max-sm:text-[4vw]  dark:border-gray-900 border-[.1vw] rounded-full p-[.1vw]" />
                    ) : (
                        <BiMoon className="font-semibold  text-[2.2vw] max-sm:text-[4vw]  dark:border-lightBlack border-[.1vw] rounded-full p-[.1vw]" />
                    )}
                </button>
                <NavLink to="/" >
                    <button className="active:scale-95 hover:text-orange-500 font-bold text-[1.1vw] max-sm:text-[2.5vw]">Home</button>
                </NavLink>
                <NavLink to="/Create">
                    <button className="active:scale-95 hover:text-orange-500 font-bold text-[1.1vw] max-sm:text-[2.5vw]">Create</button>
                </NavLink>
                <button onClick={() => dispatch(sideBar())} className="hidden max-sm:flex active:scale-95 text-center items-center">
                    <RxHamburgerMenu className="text-[3vw] max-sm:text-[5vw] ml-[2vw]" />
                </button>
            </div>
        </div>
    );
